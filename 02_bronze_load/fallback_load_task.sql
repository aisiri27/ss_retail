-- ============================================================
-- FILE NAME: fallback_load_task.sql
-- Project : S&S Retail — End-to-End Snowflake Pipeline
-- Run order: after primary_load_task.sql.
--
-- Fallback retry, 4:00pm IST (USING CRON ... Asia/Kolkata).
-- Fires only if today's PRIMARY run did NOT already succeed
-- (checked against PIPELINE_LOG inside the body). If primary
-- succeeded, exits quietly.
--
-- Load logic is identical to the primary:
--   - Transactions & Inventory: incremental COPY.
--   - Product & Store: daily snapshot via STAGING SWAP (only the
--     current-day file; real table replaced only if today's file
--     produced rows; missing file keeps prior snapshot).
--
-- CORRECTED version (staging swap). Supersedes the earlier
-- truncate-all-history version. Body matches what is deployed live.
-- ============================================================
